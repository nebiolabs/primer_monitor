...
def define_options
  Slop.parse(ARGV.map(&:strip)) do |o|
    ...
    o.string '--taxon', 'NCBI taxon id for finding taxon record'
...
def import_metadata(metadata_file, taxon)
  return unless metadata_file

  @log.info("starting import: #{metadata_file}: ")

  fasta_records = FastaRecord.parse(metadata_file, taxon)
...
def import_variants(variants_file, taxon)
  return unless variants_file

  variant_records = VariantSite.parse(variants_file, taxon)
...
def import_lineages(lineage_csv, pending, taxon, caller_id)
  return unless lineage_csv

  organism = Organism.find_by(id: taxon.organism_id)
...
def main
  ...
  taxon = OrganismTaxon.find_by(ncbi_taxon_id: opts[:taxon])

  setup_db_connection
  ActiveRecord::Base.transaction do
    if opts[:import_seqs]
      import_metadata(opts[:metadata_tsv], taxon)
      import_variants(opts[:variants_tsv], taxon)
    end
