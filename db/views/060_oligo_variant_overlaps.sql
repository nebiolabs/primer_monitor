...
SELECT variant_overlaps.oligo_id,
       ...
       generate_series(lower((numrange((variant_overlaps.oligo_start)::numeric, (variant_overlaps.oligo_end)::numeric) *
                              numrange((variant_overlaps.variant_start)::numeric,
                                       (variant_overlaps.variant_end)::numeric))),
                       (upper((numrange((variant_overlaps.oligo_start)::numeric,
                                        (variant_overlaps.oligo_end)::numeric) *
                               numrange((variant_overlaps.variant_start)::numeric,
                                        (variant_overlaps.variant_end)::numeric))) - (1)::numeric)) AS coords
FROM ((public.variant_overlaps variant_overlaps
    JOIN public.counts ON ((((counts.region)::text = (variant_overlaps.region)::text) AND
                            ((counts.subregion)::text = (variant_overlaps.subregion)::text) AND
                            ((counts.division)::text = (variant_overlaps.division)::text) AND
                            ((counts.subdivision)::text = (variant_overlaps.subdivision)::text))))
         JOIN public.time_counts ON ((((time_counts.region)::text = (variant_overlaps.region)::text) AND
                                      ((time_counts.subregion)::text = (variant_overlaps.subregion)::text) AND
                                      ((time_counts.division)::text = (variant_overlaps.division)::text) AND
