class UpdateLocationSubscriptionView < ActiveRecord::Migration[6.1]
  def up
    execute "
    CREATE OR REPLACE VIEW join_subscribed_location_to_id AS
      WITH subscribed_ids AS (
        SELECT subscribed_geo_locations.user_id, subscribed_geo_locations.detailed_geo_location_alias_id, region, subregion, division, subdivision FROM detailed_geo_location_aliases
      ...
      SELECT subscribed_ids.user_id, detailed_geo_locations.id AS detailed_geo_location_id, subscribed_ids.detailed_geo_location_alias_id FROM detailed_geo_locations
