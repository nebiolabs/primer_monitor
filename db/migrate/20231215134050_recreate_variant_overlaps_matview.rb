class RecreateVariantOverlapsMatview < ActiveRecord::Migration[6.1]
  def up
    execute <<~SQL
      ...
      CREATE MATERIALIZED VIEW IF NOT EXISTS variant_overlaps AS
          SELECT oligos.id AS oligo_id,
              ...
              JOIN oligo_alignment_positions on NOT (oligo_alignment_positions.ref_start >= variant_sites.ref_end 
                                                         or oligo_alignment_positions.ref_end <= variant_sites.ref_start) 
                                                    AND fasta_records.organism_taxon_id=oligo_alignment_positions.organism_taxon_id
