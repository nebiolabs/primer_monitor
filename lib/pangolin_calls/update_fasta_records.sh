...
\copy pangolin_calls (taxon,lineage,conflict,ambiguity_score,scorpio_call,scorpio_support,scorpio_conflict,scorpio_notes,version,pangolin_version,scorpio_version,constellation_version,is_designated,qc_status,qc_notes,note) from '$pangolin_csv' WITH (format csv, header on);
UPDATE fasta_records SET $id_field=pangolin_calls.id
